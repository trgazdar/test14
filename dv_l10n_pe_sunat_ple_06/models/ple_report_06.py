...
from .ple_report_templ import get_last_day
from .ple_report_templ import fill_name_data
from .ple_report_templ import number_to_ascii_chr
...
class PLEReport06(models.Model):
    ...
    sql_constraints = [
        ('ple_report_templ_06_unique', 'UNIQUE(year, month, company_id)',
         'Esta estructura ya está registrada para este periodo.'),
    ]

    def get_default_filename(self, ple_id='060100', empty=False):
    ...
    def generate_report(self):
        ...
        for move in lines:
            ...
            try:
                ...
                if sunat_partner_code and sunat_partner_vat:
                    ...
                    m_01.extend(['', ''])
                # 10
                m_01.append((move.move_id.l10n_latam_document_type_id.code or ''))
                # 11-12
        ...
        name_01 = self.get_default_filename(
            ple_id='060100', empty=bool(lines_to_write_01))
